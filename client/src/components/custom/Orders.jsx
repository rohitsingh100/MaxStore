  ...
  useEffect(() => {
    const fetchOrders = () => {
      try {
        axios
          .get(
            import.meta.env.VITE_API_URL +
              `/api/order/get-all-orders?page=${currentPage}&limit=10`,
            {
              headers: {
        ...
        return handleErrorLogout(error, error.response.date.message);
      }
    };
    fetchOrders();
  }, [currentPage]);
  ...
  const updateOrderStatus = async (status, paymentId) => {
    try {
      const res = await axios.put(
        import.meta.env.VITE_API_URL +
          `/api/order/update-order-status/${paymentId}`,
        { status },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      setOrders((orders) =>
        orders.map((order) =>
          order.razorpayPaymentId === paymentId ? { ...order, status: data.status } : order
        )
      );
