...
const OrderData = ({
  ...
  const handleDownloadInvoice = async () => {
    try {
      ...
      page.drawText("MaxStore", {
        x: 400,
        y: 750,
        size: 12,
        color: rgb(1, 1, 1),
      });
      page.drawText("Agra, Uttar Pradesh", { x: 400, y: 735, size: 10 });
      page.drawText("Email: rohitgola1240@gmail.com", { x: 400, y: 705, size: 10 });
      page.drawText("Phone: +919520828100", { x: 400, y: 690, size: 10 });
  ...
  return (
    <Card className="grid gap-2 p-2">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center">
        <span>
          Ordered On: <span className="capiatalize">{new Date(createdAt).toLocaleString()}</span>
        </span>
      ...
      <hr />
      <span>
        Delivery Alt: <span className="capitalize">{address}</span>
      </span>
    </Card>
  );
};
